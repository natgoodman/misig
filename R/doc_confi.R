...
doc_confi=
  function(sect=NULL,TRN=FALSE,n.fig=40,dobs.fig=0.5,dpop.fig=0.5,conf.fig=0.95) {
    ...
    figblk_end();
    if (TRN) figdev_start('plaus_confi');
    figblk_start();
...
plot_plaus=
  function(n=c(40,100,400),d.obs=0.5,howmany=c(3,1),
    ...
    vline(x=ci,y=0.05,col=ci.col,lty='dotted',lwd=2,text=round(ci,digits=2));
    ci.n=tail(n,howmany[2]); ci.col=rep(tail(col,howmany[2]),each=2);
    ci=as.vector(ci_d2t(n=ci.n,d=d.obs,conf.level=0.5));
    vline(x=ci,y=0.5,col=ci.col,lty='dotted',lwd=2,text=round(ci,digits=2));
  }
...
plot_confi=
  ...
  ci.n=head(n,howmany[1]); ci.col=rep(head(col,howmany[1]),each=2);
    ci=as.vector(ci_d2t(n=ci.n,d=d.obs,conf.level=0.95));
    vline(x=ci,y=0.95,col=ci.col,lty='dotted',lwd=2,text=round(ci,digits=2));
    ci.n=tail(n,howmany[2]); ci.col=rep(tail(col,howmany[2]),each=2);
    ci=as.vector(ci_d2t(n=ci.n,d=d.obs,conf.level=0.5));
    vline(x=ci,y=0.5,col=ci.col,lty='dotted',lwd=2,text=round(ci,digits=2));
  }
...
textd2t=function(n,d0=0,label,where=cq(above,below,left,right),cex=0.8,col='grey10') {
